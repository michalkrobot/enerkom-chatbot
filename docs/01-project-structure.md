...
```
couch/
├─ Couch.slnx                      # solution (nový .slnx formát)
...
├─ src/
│  ├─ Couch.Core/                  # sdílená knihovna (žádné HTTP)
...
└─ tests/
   ├─ Couch.Core.Tests/
   └─ Couch.Api.Tests/             # WebApplicationFactory integrační testy
```

## Projekty a jejich závislosti

```
Couch.Core   ──(referencuje)──►  nic z našich (jen NuGet)
Couch.Indexer ──►  Couch.Core
Couch.Api     ──►  Couch.Core
```

Pravidlo: **`Couch.Core` nesmí referencovat `Api` ani `Indexer`.** Vše sdílené patří do Core.
...
> **Pozn. k Azure OpenAI provideru:** Azure OpenAI má **oficiální** podporu v .NET. Použij `AzureOpenAIClient` (balíček `Azure.AI.OpenAI`) a získej `IChatClient` / `IEmbeddingGenerator` přes `Microsoft.Extensions.AI.OpenAI` (`.AsChatClient(deployment)`, `.AsEmbeddingGenerator(deployment)`). **Žádný vlastní HTTP klient není potřeba** — náš `IEmbeddingClient` / `IChatClient` z `Couch.Core/Abstractions` jsou tenké wrappery nad těmito M.E.AI rozhraními (zachovají možnost pozdější výměny providera).
>
